...
def parse_mnist(image_filename, label_filename):
    ...
    """
    ### BEGIN YOUR CODE
    # both of them are binary and .gz files
...
def softmax_loss(Z, y):
    ...
    """
    ### BEGIN YOUR CODE
    Z_sum = np.log(np.sum(np.exp(Z),axis=1))
    # fancy indexing
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(batch_num):
      ...
      Z = np.exp(X_batch @ theta)/np.sum(np.exp(X_batch @ theta), axis = 1, keepdims=True)
      grad = (X_batch.T @ (Z-np.eye(theta.shape[1])[y_batch]) )/ batch_size
      # grad = (X_batch.T @ (Z-np.eye(theta.shape[1])[y_batch]) )/  batch
      theta -= lr * grad 
    ### END YOUR CODE
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(batch_num):
      ...
      G2 = np.exp(Z1 @ W2)/np.sum(np.exp(Z1 @ W2), axis = 1, keepdims=True)\
        - np.eye(W2.shape[1])[y_batch]
      G1 = (Z1>0).astype(int) * (G2 @ W2.T)
      grad1 = (X_batch.T @ G1) / batch_size
      grad2 = (Z1.T @ G2) / batch_size
