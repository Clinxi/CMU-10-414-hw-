...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
      # calculate the adjoint(sum of partial adjoint)
      sum_grad = None
      ...
      # calculate the partial adjoint for each input nodes
      if node.is_leaf():
        continue

      i = 0
      for input_nodes in node.inputs:
        if input_nodes not in node_to_output_grads_list:
          node_to_output_grads_list[input_nodes] = []
        # print(f"{[node.op.gradient(node.grad, node)]} i: {i}")
        # try: 
        #   node_to_output_grads_list[input_nodes].append(node.op.gradient(node.grad, node)[i])
        # except:
        #   print(f"node.op.gradient(node.grad, node) {node.op.gradient(node.grad, node)}")
        # print(f"node.op.gradient(node.grad, node) {node.op.gradient(node.grad, node)}")
        node_to_output_grads_list[input_nodes].append(node.op.gradient(node.grad, node)[i]\
        if type(node.op.gradient(node.grad, node)) == tuple else node.op.gradient(node.grad, node))
        i+=1
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    topo_order = []
    visited = set()
    # get a topological sort list of nodes ending in them.
...
def topo_sort_dfs(node, visited, topo_order):
    ...
    # using dfs to get the topological sort
